...
consumer_key = "QJNijPqFCL3jnE8sLPXsBPAfK"
consumer_secret = "NR1A6lnVQrlsYVvEN1BZbd7TGyOaeKZRPXaRnLi7kD0Ve10wjW"
access_token = "334727433-hJjzBgrSsP1pS1crMqQMEH8VdNWO4ChS8P7hrK93"
access_token_secret = "rJRkPh9cvTLmjsfOHEI848atl42vnGl9J22izFXjQWOTC"
...
def get_tweets(Topic,Count):
    ...
    for tweet in tweepy.Cursor(api.search_tweets, q=Topic,count=Count-1).items():
        df.loc[i,"Date"] = str(tweet.created_at)
        df.loc[i,"User"] = str(tweet.user.screen_name)
        df.loc[i,"IsVerified"] = str(tweet.user.verified)
        df.loc[i,"Tweet"] = str(tweet.text)
        i=i+1
